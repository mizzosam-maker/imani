...
import { useAuth } from "@/context/AuthContext";
import { useRouter } from "next/navigation";
import { Search, Trash2, Edit, Shield, User as UserIcon, ChevronDown, ChevronUp, Mail, Phone, Calendar } from "lucide-react";
  ...
  return (
    <div className="container mx-auto px-4 py-4 sm:py-8">
            ...
            <tbody className="divide-y divide-[#d5c37d]">
              {filteredUsers.map((user) => (
                <tr key={user._id} className="hover:bg-[#f6e9a6] transition">
                  <td className="px-4 py-3">
                    <div className="flex items-center space-x-3">
                      <div className="w-10 h-10 bg-[#e8b924] rounded-full flex items-center justify-center flex-shrink-0">
                        <UserIcon className="w-5 h-5 text-[#0e0e10]" />
                      </div>
                      <div>
                        <div className="font-medium text-sm">{user.name}</div>
                        <div className="text-xs text-[#4d4d4d]">{user.email}</div>
                      </div>
                    </div>
                  </td>
                  <td className="px-4 py-3">
                    <div>
                      ...
                      <div className="text-xs text-[#4d4d4d]">{user.address || "—"}</div>
                    </div>
                  </td>
                  <td className="px-4 py-3">
                    <span className={`px-2 py-1 rounded-full text-xs font-medium ${
      ...
      <div className="md:hidden space-y-4">
        {filteredUsers.map((user) => (
          <div key={user._id} className="bg-[#fcf8d6] rounded-lg border border-[#d5c37d] overflow-hidden">
            {/* User Card Header */}
            <div 
            ...
            >
              <div className="flex items-center space-x-3 flex-1">
                <div className="w-12 h-12 bg-[#e8b924] rounded-full flex items-center justify-center flex-shrink-0">
                  <UserIcon className="w-6 h-6 text-[#0e0e10]" />
                </div>
                <div className="flex-1 min-w-0">
                  <div className="font-semibold text-base truncate">{user.name}</div>
                  <div className="text-xs text-[#4d4d4d] truncate">{user.email}</div>
                </div>
              </div>
            ...
            {expandedUser === user._id && (
              <div className="p-4 pt-0 space-y-3 border-t border-[#d5c37d]">
                {/* Contact Information */}
                <div className="bg-[#fafaf3] p-3 rounded-lg space-y-2">
                  <h4 className="font-semibold text-sm text-[#e8b924]">Contact Information</h4>
                  
                  <div className="flex items-center space-x-2 text-sm">
                    <Phone className="w-4 h-4 text-[#4d4d4d]" />
                    <span>{user.phone || "No phone number provided"}</span>
                  </div>
                  
                  <div className="flex items-start space-x-2 text-sm">
                    <Mail className="w-4 h-4 text-[#4d4d4d] mt-0.5" />
                ...
                {user.address && (
                  <div className="bg-[#fafaf3] p-3 rounded-lg">
                    <h4 className="font-semibold text-sm text-[#e8b924] mb-2">Address</h4>
                ...
                {/* Account Information */}
                <div className="bg-[#fafaf3] p-3 rounded-lg space-y-2">
                  <h4 className="font-semibold text-sm text-[#e8b924]">Account Information</h4>
                  ...
                  <div className="flex items-center space-x-2 text-sm">
                    <Shield className="w-4 h-4 text-[#4d4d4d]" />
                    <span>Role: {user.role === "admin" ? "Administrator" : "Regular User"}</span>
                  </div>
                </div>
                  ...
                  <button
                    ...
                    className={`flex-1 flex items-center justify-center space-x-2 px-4 py-2 rounded-lg transition ${
                      user.role === "admin" 
                        ...
                        : "bg-[#e8b924] text-[#0e0e10] hover:bg-[#ddc25d]"
                    }`}
                  >
                ...
                {user._id === currentUser?._id && (
                  <div className="text-center text-xs text-[#4d4d4d] bg-yellow-50 p-2 rounded-lg">
                    <p>⚠️ You cannot modify your own role or delete your own account</p>
                  </div>
                )}
              </div>
            )}
      ...
      {filteredUsers.length === 0 && (
        <div className="text-center py-8 sm:py-12">
          <p className="text-[#4d4d4d]">No users found matching your search</p>
        </div>
      )}
